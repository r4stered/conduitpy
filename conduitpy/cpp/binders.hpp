...
inline nb::dict joystick_to_dict(
    const org::littletonrobotics::conduit::schema::Joystick* joystick) {
  nb::dict result;

  // Convert name to string (handle null termination properly)
  std::string name_str;
  for (size_t i = 0;
       i < joystick->name()->size() && joystick->name()->Data()[i] != 0; ++i) {
...
inline nb::dict dsdata_to_dict(
    const org::littletonrobotics::conduit::schema::DSData* ds_data) {
  nb::dict result;
  ...
  for (size_t i = 0; i < ds_data->event_name()->size() &&
                     ds_data->event_name()->Data()[i] != 0;
       ++i) {
  ...
  for (size_t i = 0; i < ds_data->game_specific_message_size(); ++i) {
    game_specific_message +=
        static_cast<char>(ds_data->game_specific_message()->Data()[i]);
  }
  ...
  for (int i = 0; i < ds_data->joysticks()->size(); i++) {
    const org::littletonrobotics::conduit::schema::Joystick* joystick =
        reinterpret_cast<
            const org::littletonrobotics::conduit::schema::Joystick*>(
            &ds_data->joysticks()->Data()[i]);
...
inline nb::dict pdpdata_to_dict(
    const org::littletonrobotics::conduit::schema::PDPData* pdp_data) {
  nb::dict result;
...
inline nb::dict canstatus_to_dict(
    const org::littletonrobotics::conduit::schema::CANStatus* can_status) {
  nb::dict result;
...
inline nb::dict systemdata_to_dict(
    const org::littletonrobotics::conduit::schema::SystemData* sys_data) {
  nb::dict result;
...
inline nb::dict coreinputs_to_dict(
    const org::littletonrobotics::conduit::schema::CoreInputs* core_inputs) {
  nb::dict result;
...
class PyWPILibIO {
 public:
  PyWPILibIO() { akit::conduit::wpilibio::make_buffer(); }

  void start() { akit::conduit::wpilibio::start(); }

  nb::dict capture_data() {
    akit::conduit::wpilibio::capture_data();
    org::littletonrobotics::conduit::schema::CoreInputs* data =
        static_cast<org::littletonrobotics::conduit::schema::CoreInputs*>(
            akit::conduit::wpilibio::shared_buf);
    return coreinputs_to_dict(data);
  }

  ~PyWPILibIO() {
    if (akit::conduit::wpilibio::shared_buf) {
      std::free(akit::conduit::wpilibio::shared_buf);
